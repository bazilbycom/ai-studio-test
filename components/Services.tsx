    ...
    <section id="services" className="py-12 md:py-24 px-6 relative">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 md:mb-20 gap-8">
          ...
          <motion.p initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-zinc-500 max-w-sm text-left md:text-right font-medium hidden md:block">
            High-precision digital tools for business acceleration.
          </motion.p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {servicesData.map((s, idx) => (
            <motion.div key={idx} whileHover={{ y: -8 }} className="glass-panel p-8 rounded-[2rem] border border-white/5 hover:border-[#10b981]/40 transition-all">
              <span className="text-3xl font-black text-white/5 mb-6 block">{s.num}</span>
              ...
              <p className="text-zinc-500 text-xs leading-relaxed mb-6">{s.desc}</p>
